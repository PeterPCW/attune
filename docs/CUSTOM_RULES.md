...
```bash
# Load custom rules from a single file
attune analyze . --rules-path ./my-rules.json

# Load custom rules from a directory
attune analyze . --rules-path ./my-rules/

# Combine with other options
attune analyze . --rules-path ./rules/ --json --output results.json
```

Custom rules are merged with built-in rules and run together.
